...
월드컵 경기 일정과 경기 결과 카드뉴스 제작에 필요한 데이터를 API-Football에서 수집하고, 관리자 화면에서 검수 가능한 형태로 저장한 뒤, 카드뉴스 제작 화면의 날짜 선택만으로 카드 편집기에 자동 입력되도록 만든다.
...
- API-Football 경기 데이터 자동화 대상이 아니다.
...
- API-Football 월드컵 fixture 데이터를 사용한다.
...
## 3. API-Football 사용 기준

### 월드컵 식별자

- API-Football 월드컵 2026 기준:
  - `league=1`
  - `season=2026`
- 주요 엔드포인트:
  - `/fixtures`
...
- 예시 기준:
  ...
  - `to=YYYY-MM-DD`
  - `timezone=Asia/Seoul`
- API에서 받은 날짜/시간은 카드뉴스 운영 기준인 KST로 저장 또는 변환한다.

### 결과 조회

- 일정과 같은 `/fixtures` 데이터를 사용한다.
...
- API-Football의 `fixture.id`를 `api_fixture_id`로 저장한다.
- `api_fixture_id` 기준으로 upsert한다.
- 이미 저장된 경기면 업데이트하고, 새로운 경기면 추가한다.
- 카드뉴스 제작 화면은 API-Football을 직접 호출하지 않고, 우리 DB만 읽는다.
...
API 원본값과 카드뉴스 표시값은 분리해서 저장하는 것이 좋다.
...
이렇게 해야 API 데이터가 갱신되어도 카드뉴스 편집/표시 정책을 안정적으로 유지할 수 있다.
...
2. API-Football `/fixtures`를 호출한다.
3. `league=1`, `season=2026`, `from/to`, `timezone=Asia/Seoul` 기준으로 조회한다.
4. 받은 fixture 데이터를 `world_cup_fixtures`에 upsert한다.
...
- API-Football에서 가져온 데이터가 DB에 제대로 저장되었는지 확인한다.
...
## 12. API 데이터와 카드뉴스 필드 매핑

| 카드뉴스 필드 | API-Football 또는 내부 변환 |
| --- | --- |
| `kickoff_time` | `fixture.date`를 KST HH:mm으로 변환 |
| `time_period` | KST 시간이 12시 전이면 `오전`, 이후면 `오후` |
| `home_team` | 내부 한국어 국가명 매핑 |
| `away_team` | 내부 한국어 국가명 매핑 |
| `home_code` | API 팀 코드 또는 내부 국가 코드 매핑 |
| `away_code` | API 팀 코드 또는 내부 국가 코드 매핑 |
| `home_image_url` | 내부 국기 URL 또는 API 팀 로고 |
| `away_image_url` | 내부 국기 URL 또는 API 팀 로고 |
| `group_label` | `/standings` 또는 내부 그룹 매핑 |
| `venue` | 경기장 한글명 매핑, 없으면 API 경기장명 |
| `home_score` | `goals.home` 또는 확정 스코어 |
| `away_score` | `goals.away` 또는 확정 스코어 |
...
- API fixture의 `league.round`만으로는 `Group A`를 안정적으로 얻지 못할 수 있다.
- `/standings`에서 팀의 그룹을 가져와 홈/원정 팀 기준으로 매칭하는 방식이 좋다.
...
- API-Football은 월드컵 일정/결과 자동화에 사용한다.
- 일정 동기화 버튼과 결과 동기화 버튼은 분리한다.
- 저장 데이터는 같은 fixture 레코드에 누적한다.
- 카드뉴스 제작 화면은 API-Football을 직접 호출하지 않고 DB 데이터를 읽는다.
